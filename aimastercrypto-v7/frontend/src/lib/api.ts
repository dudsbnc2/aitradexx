...
export const login = async (email: string, password: string) => {
  // V7: usar endpoint seguro que coloca refresh token em httpOnly cookie
  const r = await api.post('/v1/auth/login-secure', { email, password })
  const data = r.data
...
export const register = async (email: string, username: string, password: string) => {
  // Registo continua a usar o endpoint original (não tem versão segura ainda)
...
export const logout = async () => {
  try {
    // V7: invalidar refresh token no backend
    await api.post('/v1/auth/logout-secure')
