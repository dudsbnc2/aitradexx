...
Price: {ind['price']} | ATR(14): {ind['atr']} | Regime: {ind.get('atr_regime','NORMAL')}
...
{{"bias":"LONG","confidence":72,"rating":"GOOD","entry":{ind['price']},"entryZoneHigh":0,"stopLoss":0,"takeProfit":0,"invalidation":0,"rr":"1:2.0","analysis":"2-3 sentences referencing actual values.","scenarios":[{{"name":"Bull Case","prob":55,"target":0,"invalidation":0,"description":"one sentence"}},{{"name":"Base Case","prob":30,"target":0,"invalidation":0,"description":"one sentence"}},{{"name":"Bear Case","prob":15,"target":0,"invalidation":0,"description":"one sentence"}}],"indicators":{{"ema":"{ind['ema_signal']}","rsi":{int(ind['rsi'])},"macd":"{ind['macd_signal_dir']}","volume":"{ind['volume']}","volMult":{ind['vol_mult']},"structure":"{ind['structure']}","vwap":"{ind['vwap_signal']}"}},"tags":["tag1","tag2","tag3"]}}

RULES: bias LONG/SHORT/WAIT. SL=1.5×ATR. TP=3×ATR. probs sum to 100. LONG/SHORT only when ≥3 indicators align."""
...
async def get_ai_signal(pair: str, tf: str, ind: dict, mtf: Optional[dict] = None) -> tuple[dict, str]:
    ...
    if settings.GEMINI_API_KEY:
        try:
            ...
            return enrich_signal(s, ind), "gemini-1.5-flash"
        except Exception as e:
